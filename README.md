...
- **Roster grande**: más de 130 familias de criaturas invocables (137 con las 15 originales) con 3 formas cada una. Los enemigos y jefes del mapa son contenido aparte, no jugable ni invocable: 33 familias de enemigos normales de relleno y 33 jefes únicos (uno por zona, sin evolución), cada uno con su elemento, clase, habilidad y una frase de historia propia — ninguno de los dos aparece nunca en la Colección, la Invocación ni la Arena.
...
- **Mapa por etapas, cada una un recorrido**: 33 zonas con 8 etapas cada una. Las etapas 1-7 son un recorrido de varios encuentros (siempre 3 rivales de relleno propios de esa zona por oleada) seguido de un nodo de recompensa — no es un único combate: hay que superar todos los encuentros para cobrar la recompensa. La 8ª etapa es el jefe de zona: un combate único, solo contra él, con toda tu banda. Cada jefe tiene la misma rareza que los enemigos de relleno de su propia zona (así escala junto con el resto del progreso) y un +140% de vida extra para que la pelea dure varias rondas de refuerzos en vez de resolverse de un golpe. La vida y los desmayos de la banda persisten entre nodos de un mismo recorrido (ya no se cura sola al pasar de encuentro), así que llegar mal parado al siguiente combate es un riesgo real.
...
- **Equipo**: 6 huecos por luchador — arma, armadura, casco, guantes, botas y amuleto —, cada uno con una estadística principal y una secundaria más floja propias, mejorables con Texel.
