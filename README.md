...
- **Mapa por etapas, cada una un recorrido**: 6 zonas con 8 etapas cada una (la 8ª es el jefe de zona). Entrar en una etapa cuesta Energía (se regenera con el tiempo) y abre un recorrido con varios nodos de combate seguidos de un nodo de recompensa — no es un único combate: hay que superar todos los encuentros del recorrido para cobrar la recompensa de la etapa. La vida y los desmayos de la banda persisten entre nodos de un mismo recorrido (ya no se cura sola al pasar de encuentro), así que llegar mal parado al siguiente combate es un riesgo real.
