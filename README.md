...
- **Mapa por etapas, cada una un recorrido**: 33 zonas con 8 etapas cada una. Las etapas 1-7 son un recorrido de varios encuentros (siempre 3 rivales por oleada) seguido de un nodo de recompensa — no es un único combate: hay que superar todos los encuentros para cobrar la recompensa. La 8ª etapa es el jefe de zona: un combate único, solo contra él — cada uno de los 27 jefes creados tiene su propia zona. La vida y los desmayos de la banda persisten entre nodos de un mismo recorrido (ya no se cura sola al pasar de encuentro), así que llegar mal parado al siguiente combate es un riesgo real.
